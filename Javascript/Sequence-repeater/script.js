...
function isTheRightCombination(position) {
  let computerCombination = START_REACTOR.computerCombination.slice(0, position);
...
function play(idx) {
  ...
  if (isTheRightCombination(START_REACTOR.playerCombination.length)) {
    if (START_REACTOR.playerCombination.length == START_REACTOR.combinationMaxPosition) {
      endGame('complete');
      setTimeout(() => {
        start();
      }, 1200);
